...
export const ProfileSection: React.FC<ProfileSectionProps> = ({ 
  ...
  const handleTabSelect = (tab: 'profil' | 'visi' | 'adart' | 'pengurus') => {
    ...
    // If user has scrolled down into content, keep the tab bar comfortably in view
    const tabEl = document.getElementById('profil-tabs') || document.getElementById('profil');
    if (tabEl) {
      const rect = tabEl.getBoundingClientRect();
      if (rect.top < 64) {
        const targetY = Math.max(0, Math.round(rect.top + window.scrollY - 78));
    ...
    <section id="profil" className="py-12 sm:py-16 md:py-24 bg-white border-y border-slate-100 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-8 sm:mb-10">
          ...
          <div id="profil-tabs" className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 mt-6 sm:mt-8 p-1.5 bg-slate-100/90 rounded-2xl max-w-2xl mx-auto border border-slate-200/80 shadow-xs">
            <button
              id="tab-btn-profil"
        ...
        {/* Tab Content Panels with Stable Minimum Height */}
          ...
          {currentTab === 'profil' && (
            <div className="animate-tab-fade grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
              ...
              <div className="md:col-span-1 lg:col-span-6">
                <div className="relative rounded-2xl overflow-hidden border border-emerald-200/80 shadow-xl bg-slate-900">
                  <img
              ...
              <div className="bg-emerald-50/50 rounded-2xl p-4.5 sm:p-6 md:p-8 border border-emerald-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                  {visionMission.misi.map((tujuan, idx) => (
                    <div 
                      key={idx} 
                      className="bg-white p-3.5 sm:p-4.5 rounded-xl border border-slate-200/80 flex items-start gap-2.5 sm:gap-3 shadow-2xs"
                    >
              ...
              <div className="space-y-3 sm:space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                  {visionMission.pillars.map((pillar, idx) => (
                    <div 
                      key={idx} 
                      className="p-3.5 sm:p-4 rounded-xl bg-white border border-slate-200"
                    >
